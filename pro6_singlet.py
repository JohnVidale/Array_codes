...
#%% -- Count slownesses
    ...
    ttt             = start_buff + (np.arange( st[0].stats.npts) * st[0].stats.delta) # time array for inputs
    cc_ttt_full     = start_buff + (np.arange((st[0].stats.npts-1)/dec_fac + 1) * st[0].stats.delta * dec_fac) # time array for outputs
