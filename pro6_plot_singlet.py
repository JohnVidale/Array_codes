...
def pro6stacked_singlet(eq_file, plot_scale_fac = 0.03, slow_delta = 0.0005,
              ...
              norm = 0, dphase = 'PKiKP', take_median = 0, log_plot = 0,
...
#    event_obs_bazi  = event_baz[ii] + (bazi_rad * 180 / np.pi)
    if NS == 1:
        print('PN '+ str(pred_Nslo) + ' PE ' + str(pred_Eslo) + ' ON ' + str(obs_Nslo) + ' OE ' + str(obs_Eslo))
    else:
...
#%% Plot R and T amp trace vs time plots
    if plot_trace_sums == 1:
        # Plot radial amp vs time plots
        fig_index = 7
        ...
        # Plot transverse amp vs time plots
        fig_index = 8
        plt.figure(fig_index,figsize=(30,10))
        plt.xlim(start_buff,end_buff)
...
#%% plot R-T amplitude averaged over time window
    fig_index = 9
    ...
    for slowR_i in range(slowR_n):  # loop over radial slownesses
        for slowT_i in range(slowT_n):  # loop over transverse slownesses
            ...
            if take_median == 1:  # median or average?
                if full_beam == 1:
    ...
    if log_plot == 1:
        c = ax.pcolormesh(x1, y1, stack_slice, cmap=plt.cm.gist_rainbow_r, vmin = stack_slice)
