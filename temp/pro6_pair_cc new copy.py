...
def pro6_cc_pair(eq_file1, eq_file2, plot_scale_fac = 0.03, slow_delta = 0.0005,
              ...
              cc_twin = 2, cc_len = 0.5, cc_interp1d = 5, cc_delta = 0.1):
...
#%% Get info
    ...
    # Make matrix for old time shifts
    ...
    # Make matrix for cc time shifts
    tshift_new    = st1.copy()  # make array for cc time shift
    cc_new        = st1.copy()  # make array for cc coefficient
    # Trim off ends correlation cannot reach
    border_trim = cc_twin/2 + cc_delta
    time_s = st1[0].stats.starttime + start_buff + border_trim
    time_e = st1[0].stats.starttime + end_buff - (2 * border_trim)
    tshift_new.trim(time_s, time_e)
    cc_new.trim(    time_s, time_e)
...
#%%  Loop over slowness
    ...
    for slow_i in range(total_slows): # find envelope, phase, tshift, and global max
        ...
        tr1 = st1[slow_i]
        tr2 = st2[slow_i]
        cc_ttt, cc_coef, tshift_vec = cc_measure_tshift( tr1=tr1, tr2=tr2, tarr_beg = twin_beg,
                          cc_twin=cc_twin, cc_len=cc_len, cc_delta=cc_delta, cc_interp1d=cc_interp1d)

        print(f'd_phase {len(d_phase)} tshift_vec {len(tshift_vec)}]')
        tshift_new[slow_i].data = tshift_vec
        cc_new[slow_i].data     = cc_coef
        ...
        if (pt1 == True) or ( pt2 == True):
            ...
            if pt2 == True:
                plt.figure(5)
            plt.title('Radial slowness ' + str(SloR) + '  Trans slowness ' + str(SloT))
            plt.subplot(3,1,1)
            plt.xlim(start_buff,end_buff)
            plt.plot(cc_ttt,cc_coef)  # cross correlation
            plt.subplot(3,1,2)
            plt.xlim(start_buff,end_buff)
            plt.plot(ttt,tshift[slow_i].data,'k')  # old time shift
            plt.plot(cc_ttt,tshift_new,'r')  # new time shift
            zero_line = tshift_new * 0
            plt.plot(cc_ttt,zero_line,'y')  # new time shift
            plt.subplot(3,1,3)
            plt.xlim(start_buff,end_buff)
            plt.plot(ttt,st1[slow_i].data)  # traces for 1st event
            plt.plot(ttt,st2[slow_i].data)  # traces for 2nd event
            # Mark all the cc>0.8 points
            nn=[index for index,x in enumerate(cc_coef) if x>=0.8]
            #cc_tshift[nn]=np.nan
            plt.subplot(3,1,1)
            plt.scatter(cc_ttt[nn],cc_coef[nn],color='k',s=20)
            plt.subplot(3,1,2)
            plt.scatter(cc_ttt[nn],tshift_new[nn],color='g',s=20)
...
#%% Label unreliable points as nan
    # process old time shifts
    tshift_full = tshift.copy()  # make array for time shift
    for slow_i in range(total_slows): # ignore less robust points
        if slow_i % 200 == 0:
            print('Labeling nan in old tshift, ' + str(slow_i) + ' finished slownesses out of ' + str(total_slows))
        for it in range(nt1):
    ...
    # process new time shifts
    tshift_new_full = tshift_new.copy()  # make array for time shift
    for slow_i in range(total_slows): # ignore less robust points
        if slow_i % 200 == 0:
            print('Labeling nan in new tshift, ' + str(slow_i) + ' finished slownesses out of ' + str(total_slows))
        for it in range(nt1):
            if cc_coef < 0.8:
                tshift_new[slow_i].data[it] = np.nan
