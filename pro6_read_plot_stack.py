...
#			ids.append(split_line[0])  ignore label for now
	...
	#%%  Plotting
	stack_array = np.zeros((total_slows,stack_nt))

	min_allowed = global_max/plot_dyn_range
	for it in range(stack_nt):  # check points one at a time
		for slow_i in range(total_slows):  # for this station, loop over slownesses
			num_val = st[slow_i].data[it]
			if num_val < min_allowed:
				num_val = min_allowed
			stack_array[slow_i, it] = math.log10(num_val)

	y, x = np.mgrid[slice(0, total_slows, 1),
