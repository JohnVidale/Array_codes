...
inventory = client.get_stations(longitude=ev_lon, latitude=ev_lat, starttime =
                                ...
                                station = sta_list, channel = '*Z', network = network_list,
...
for tr in st:# keep only a single location per station
        ...
        tr.taper(0.05)
        tr.filter('bandpass', freqmin=1, freqmax=4, corners=4, zerophase=False)
        if reject == False:
...
mseed_name1 = year_label + month_label + day_label + '_' + hour_label + minute_label
