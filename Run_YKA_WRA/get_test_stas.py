#!/usr/bin/env python
# John Vidale 7/2022
# gets SHZ stations from several air force arrays

def get_test_stas(eq_num = 401, fig_index = 1, chosen_network = 'IM'):
    ...
    chan_type  = 'HNZ,EHZ,HHZ,HLZ,BHZ,SHZ' # e.g., BHZ
    ...
    #%% Make inventory of all stations recording this channel for this event
    inventory = client.get_stations(longitude=ev_lon, latitude=ev_lat, starttime =
                                    ...
                                    station = '*', channel = '*Z', network = chosen_network,
    ...
    for network in inventory:
        if network.code == chosen_network:  # Pick network
            for station in network:
                cnt = cnt + 1
                if cnt%10 == 0:
                    print(str(cnt) + ' stations examined, ' + str(len(st)) + ' traces extracted')
                try:
                    # print(str(cnt) + ' network ' + network.code + ' station ' + station.code + ' trace attempted')
                    st += client.get_waveforms(network.code, station.code, location='*',channel='*Z', starttime=s_t, endtime = e_t, attach_response=True)
                    # print('Worked: ' + str(len(st)) + ' network ' + st[-1].stats.network + ' station ' + st[-1].stats.station + ' location ' + st[-1].stats.location + ' channel ' + st[-1].stats.channel)
                except:
                    pass
    ...
    print('Total channels ' + str(len(st)) + ' - HNZ, EHZ, HHZ, HLZ, BHZ, SHZ, other have '
           + str(hnz_chosen) + ' ' + str(ehz_chosen) + ' '
           + str(hhz_chosen) + ' ' + str(hlz_chosen) + ' ' + str(bhz_chosen) + ' ' + str(shz_chosen) + ' ' + str(other_chosen))
    ...
    write_name = '/Users/vidale/Documents/Research/IC/Mseed/Sm_array/' + mseed_name + '.mseed'
