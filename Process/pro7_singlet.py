...
def pro7_singlet(eq_num, slow_delta = 0.0005, turn_off_black = 0,
    ...
    print(f'amp_ave data length is {nt_amp} time pts, dt is {dt:.4f}, so record length is {dt * nt   :.0f} seconds')
    ...
    if log_plot:  # convert amp envelope array to log amp and record global_max of logs
        ...
        for slow_i in range(len(amp_ave)): # find global max of amp_ave
            for data_i in range(len(amp_ave[slow_i].data)): # find global max of amp_ave
