...
from pro6_plot_pair          import pro6stacked_pair  #renamed, remove this comment if runs correctly
...
cc_len       =  0.3;        # time window shift to compute CC (fraction of whole time window)
cc_delta     =  0.1;      # time interval for cc (s)
cc_interp1d  = 10;      # interpolation factor
...
stat_corr = 1
decimate_fac   =    5 # applies to output from pro5stack2d, set to 0 for other codes
simple_taper   =    1
skip_SNR       =    1
skip_snaps     =    1
snaptime       = 1204
...
##%% --Old version - Compare pair of 2D stack results to find shift, amp, amp ratio
# pro6stacked_pair(eq_file1 = eq_file1, eq_file2 = eq_file2, plot_scale_fac = 0.002,
#             slowR_lo = slowR_lo, slowR_hi = slowR_hi, slowT_lo = slowT_lo, slowT_hi = slowT_hi, slow_delta = slow_delta,
#             start_buff = start_buff, end_buff = end_buff, freq_corr = freq_corr, dphase = ref_phase,
#             fig_index = 301, plot_dyn_range = 100, ARRAY = ARRAY, decimate_fac = 0,
#             turn_off_black = 0, R_slow_plot = R_slow_plot, T_slow_plot = T_slow_plot,
#             max_rat = max_rat, min_amp = min_amp, tdiff_clip = tdiff_clip)

##%% --New cc version - Compare pair of 2D stack results to find shift, amp, amp ratio, uses cc rather than instant phase
pro6_cc_pair(eq_file1 = eq_file1, eq_file2 = eq_file2, plot_scale_fac = 0.002,
            slowR_lo = slowR_lo, slowR_hi = slowR_hi, slowT_lo = slowT_lo, slowT_hi = slowT_hi, slow_delta = slow_delta,
            start_buff = start_buff, end_buff = end_buff, freq_corr = freq_corr, dphase = ref_phase,
            ARRAY = ARRAY, decimate_fac = 0, turn_off_black = 0, R_slow_plot = R_slow_plot, T_slow_plot = T_slow_plot,
            max_rat = max_rat, min_amp = min_amp, tdiff_clip = tdiff_clip, cc_twin = cc_twin, cc_len = cc_len,
            cc_interp1d = cc_interp1d)
...
#             start_buff = start_buff, end_buff = end_buff, skip_T = 1, skip_R = 1, skip_snaps = skip_snaps, tdiff_clip = tdiff_clip,
#             fig_index = 301, plot_dyn_range = 100, snaptime = snaptime, snaps=snaps, decimate_fac = 0,
